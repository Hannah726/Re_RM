...
class EHR(object):
    ...
    def prepare_tasks(self, cohorts, spark, cached=False):
        ...
        if self.cfg.use_ed:
            ...
            labeled_cohorts = labeled_cohorts.drop(columns=["LOS"])
    ...
    def process_tables(self, cohorts, spark):
        ...
        events_dfs = []
        table_metadata = dict()  # Store table metadata 
        for table in self.tables:
            ...
            # XXX Convert numeric columns to proper numeric types and handle non-numeric values
            # This should be done before tokenization
            for col in table["numeric"]:
                if col in events.columns:
                    # First, replace empty strings and whitespace with null
                    events = events.withColumn(col, 
                        F.when((F.col(col) == "") | (F.col(col) == " "), None)
                        .otherwise(F.col(col))
                    )
                    
                    # Try to cast to double, non-numeric values will become null
                    events = events.withColumn(col, F.col(col).cast("double"))
            ...
            table_metadata[lower_table_name] = {
                'pid_column': table.get("pid", "stay_id"),
                'item_column': table.get("item", None),
                'item_unrelated': table.get("item_unrelated", []),
                'time_column': table.get("time", "time"),
                'categorical_columns': table["categorical"],
                'numeric_columns': table["numeric"],
            }
            
        # Save table metadata to pickle file
        metadata_file = os.path.join(self.dest, f"{self.ehr_name}_table_metadata.pkl")
        with open(metadata_file, 'wb') as f:
            pickle.dump(table_metadata, f)
        logger.info(f"Table metadata saved to {metadata_file}")
